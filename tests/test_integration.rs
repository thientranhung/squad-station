...
fn test_init_hook_prints_instructions() {
    let tmp = tempfile::TempDir::new().unwrap();
    let db_file = tmp.path().join("station.db");
    // Use an unknown provider so no hooks are auto-installed,
    // forcing the manual hook instructions to be printed.
    let yaml = "project: test-squad\norchestrator:\n  name: test-orch\n  provider: unknown-provider\n  role: orchestrator\nagents: []\n";
    std::fs::write(tmp.path().join("squad.yml"), yaml).unwrap();
